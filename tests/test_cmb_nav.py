...
def test_planck_2018_velocity_matches_quoted_speed():
    ...
    assert np.isclose(np.linalg.norm(v), 369.82)


def test_sky_sample_directions_are_unit_vectors_well_spread():
    dirs = sky_sample_directions(16)
    assert np.allclose(np.linalg.norm(dirs, axis=1), 1.0)
    # well-spread samples should not be rank-deficient
    assert np.linalg.matrix_rank(dirs) == 3


def test_dipole_temperature_matches_definition():
    velocity = np.array([1.0, 0.0, 0.0]) * C_KM_S * 0.01  # v/c = 0.01 along x
...
def test_velocity_fix_at_50uK_noise_is_sub_degree():
    rng = np.random.default_rng(7)
    true_velocity = planck_2018_velocity_vector()
    result = velocity_fix(true_velocity, noise_kelvin=50e-6, rng=rng)
    assert result["direction_error_deg"] < 1.0
